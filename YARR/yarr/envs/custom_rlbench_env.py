...
class CustomRLBenchEnv(RLBenchEnv):
    ...
    def extract_obs(self, obs: Observation, t=None, prev_action=None):
        ...
        # print(obs_dict['lang_goal'], obs_dict['gripper_pose'],obs_dict['gripper_open'])
        ...
        obs.gripper_pose = grip_pose
        obs_dict['gripper_pose'] = grip_pose # uncommented, hope this will give us back the gripper position
    ...
    def reset_to_demo(self, i):
        ...
        _, obs = self._task.reset_to_demo(d) # this obs is very buggy. It is the one we store locally not the new episode
        self._lang_goal = self._task.get_task_descriptions()[0]

        self._previous_obs_dict = self.extract_obs(self._task._scene.get_observation()) #
