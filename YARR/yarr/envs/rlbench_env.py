...
def _extract_obs(obs: Observation, channels_last: bool, observation_config):
    ...
    for config, name in [
        ...
        if config.mask:
            obs_dict['%s_mask_id_to_name' % name] = obs.misc['%s_mask_id_to_name' % name] ## add mapping between mask ad object id
...
class RLBenchEnv(Env):
    ...
    def extract_obs(self, obs: Observation):
        ...
        if self._include_lang_goal_in_obs:
            ...
            extracted_obs['lang_goal'] = self._lang_goal ### added!
            extracted_obs['gripper_pose'] = obs.gripper_pose ### added!
            extracted_obs['gripper_open'] = np.array([obs.gripper_open]) ### added!
            # print("sasdasdasdasddasd", self._lang_goal,obs.gripper_pose,obs.gripper_open)
