...
class _IndependentEnvRunner(_EnvRunner):
    ...
    def _run_eval_independent(self, name: str,
        ...
        real_task_name, _ = self._get_task_name()
        ...
        if type(weight) == int:
            ...
            self._agent.load_weights(self._logdir, components)
        ...
        for n_eval in range(self._num_eval_runs):
            ...
            if type(weight) == dict:
                # print(list(weight.keys()),"?"*20, n_eval, self._num_eval_runs)
                ...
                print('Evaluating weight %s for %s' % (weight_name, real_task_name))
                logging.info('Evaluating weight %s for %s' % (weight_name, real_task_name))
            ...
            for ep in range(self._eval_episodes):
                ...
                print(f"Evaluating {task_name} | Episode {ep} | Step: {eposide_step} | Score: {reward} | Lang Goal: {lang_goal}")
                logging.info(f"Evaluating {task_name} | Episode {ep} | Step: {eposide_step} | Score: {reward} | Lang Goal: {lang_goal}")
                writer.add_log(f"Evaluating {task_name} | Episode {ep} | Step: {eposide_step} | Score: {reward} | Lang Goal: {lang_goal}")
            ...
            writer.add_log(f"Finished {eval_task_name} | Final Score: {task_score}")
