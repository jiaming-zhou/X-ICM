...
class Scene(object):
    ...
    def get_observation(self) -> Observation:
        ...
        def get_mask(sensor: VisionSensor, mask_fn):
            mask, mask_id_to_name = None, None
            if sensor is not None:
                sensor.handle_explicitly()
                raw_masks = sensor.capture_rgb()
                mask = mask_fn(sensor.capture_rgb())
                mask_id_to_name = sensor.get_mask_id_object_names(rgb_handles_to_mask(raw_masks)) # a dictionary
            return mask, mask_id_to_name
        ...
        left_shoulder_mask, left_shoulder_mask_id_to_name = get_mask(self._cam_over_shoulder_left_mask,
                                                                     lsc_mask_fn) if lsc_ob.mask else (None, None)
        right_shoulder_mask, right_shoulder_mask_id_to_name = get_mask(self._cam_over_shoulder_right_mask,
                                                                       rsc_mask_fn) if rsc_ob.mask else (None, None)
        overhead_mask, overhead_mask_id_to_name = get_mask(self._cam_overhead_mask,
                                                           oc_mask_fn) if oc_ob.mask else (None, None)
        wrist_mask, wrist_mask_id_to_name = get_mask(self._cam_wrist_mask,
                                                     wc_mask_fn) if wc_ob.mask else (None, None)
        front_mask, front_mask_id_to_name = get_mask(self._cam_front_mask,
                                                     fc_mask_fn) if fc_ob.mask else (None, None)

        misc_dict = self._get_misc()
        misc_dict["left_shoulder_mask_id_to_name"] = left_shoulder_mask_id_to_name
        misc_dict["right_shoulder_mask_id_to_name"] = right_shoulder_mask_id_to_name
        misc_dict["overhead_mask_id_to_name"] = overhead_mask_id_to_name
        misc_dict["wrist_mask_id_to_name"] = wrist_mask_id_to_name
        misc_dict["front_mask_id_to_name"] = front_mask_id_to_name
