...
class VisionSensor(Object):
    ...
    def get_mask_id_object_names(self, mask_ids: np.ndarray) -> dict:
        """Retrieves object names from mask ids. Note if a name is an empty string, it means nothing is found to correspond to the mask id.

        :param mask_ids: A numpy array of arbitrary size, containing mask ids.
        :return: A dictionary that maps mask ids to object names
        """
        uniques = np.unique(mask_ids)
        mapping = {}
        for mask_id in uniques:
            mapping[mask_id] = sim.simGetObjectName(mask_id)
        return mapping
